...
void Draw()
{
  ...
  if ( time > 255 )
  {
    ...
    for ( int i = -2; i <= 800 / 32 + 1; i++ )
      asprite2d_Draw( texGround, i * 32.0f, 96 - 12, 32, 32, 0, 2, 255, FX_BLEND );
    for ( int i = -2; i <= 800 / 32 + 1; i++ )
      asprite2d_Draw( texGround, i * 32.0f, 600 - 32 - 12, 32, 32, 0, 2, 255, FX_BLEND );
    ...
    for ( int i = 0; i < 10; i++ )
      if ( i == 2 )
      {
        ...
        pr2d_Rect( tux[ i ].Pos.X - 2, tux[ i ].Pos.Y - fntMain->MaxHeight + 4, t, (float)fntMain->MaxHeight, 0xFFFFFF, 255, 0 );
        text_DrawEx( fntMain, tux[ i ].Pos.X, tux[ i ].Pos.Y - fntMain->MaxHeight + 8, 0.75, 0, "I'm so red...", 255, 0xFFFFFF, 0 );
      ...
      else if ( i == 7 )
      {
        ...
        pr2d_Rect( tux[ i ].Pos.X + 32 - t / 2, tux[ i ].Pos.Y - fntMain->MaxHeight + 4, t, (float)fntMain->MaxHeight, 0xFFFFFF, 255, 0 );
        ...
        asprite2d_Draw( tux[ i ].Texture, tux[ i ].Pos.X, tux[ i ].Pos.Y, 64, 64, 0, tux[ i ].Frame / 2, 255, FX_BLEND );
    ...
    for ( int i = 10; i < 20; i++ )
      if ( i == 13 )
      {
        ...
        pr2d_Rect( tux[ i ].Pos.X - 2, tux[ i ].Pos.Y - fntMain->MaxHeight - 10, t, (float)fntMain->MaxHeight, 0xFFFFFF, 255, 0 );
        text_DrawEx( fntMain, tux[ i ].Pos.X, tux[ i ].Pos.Y - fntMain->MaxHeight - 4, 0.75, 0, "I'm so big...", 255, 0xFFFFFF, 0 );
    ...
    asprite2d_Draw( texGround, 13 * 32, 300 - 16, 32, 32, 0, 3, 255, FX_BLEND );
    ...
    pr2d_Rect( tux[ 20 ].Pos.X + 32 - t / 2, tux[ 20 ].Pos.Y - fntMain->MaxHeight + 4, t, (float)fntMain->MaxHeight, 0xFFFFFF, 255, 0 );
    text_DrawEx( fntMain, tux[ 20 ].Pos.X + 32, tux[ 20 ].Pos.Y - fntMain->MaxHeight + 8, 0.75, 0, "o_O", 255, 0xFFFFFF, TEXT_HALIGN_CENTER );
    asprite2d_Draw( tux[ 20 ].Texture, tux[ 20 ].Pos.X, tux[ 20 ].Pos.Y, 64, 64, 0, tux[ 20 ].Frame / 2, 255, FX_BLEND );
  }

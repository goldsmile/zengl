...
void Init()
{
	...
	icon[ 0 ] = tex_LoadFromFile( GetResource( "audio-stop.png" ) );
	icon[ 1 ] = tex_LoadFromFile( GetResource( "audio-play.png" ) );
...
void Draw()
{
	...
	ssprite2d_Draw( icon[ state ], ( SCREEN_WIDTH - 128 ) / 2.f, ( SCREEN_HEIGHT - 128 ) / 2.f, 128.f, 128.f, 0 );
	...
	if ( col2d_PointInRect( (float)mouse_X(), (float)mouse_Y(), r ) )
	{
		...
		ssprite2d_Draw( icon[ state ], ( SCREEN_WIDTH - 132 ) / 2.f, ( SCREEN_HEIGHT - 132 ) / 2.f, 132.f, 132.f, 0, 155 );
...
void Timer()
{
	...
	if ( mouse_Click( M_BLEFT ) )
	{
		...
		snd_Play( sound, FALSE, CalcX2D( (float)mouse_X() ), CalcY2D( (float)mouse_Y() ) );
		...
		if ( col2d_PointInRect( (float)mouse_X(), (float)mouse_Y(), r ) && ( audio == 0 ) )
			audio = snd_PlayFile( GetResource( "music.ogg" ) );
	}

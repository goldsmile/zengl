...
void Init()
{
	texTux = tex_LoadFromFile( GetResource( "tux_stand.png" ) );
...
void Draw()
{
	...
	// RU: Рисуем в него.
	// EN: Render to it.
	asprite2d_Draw( texTux, float(rand() % ( 800 - 64 )), float(rand() % ( 600 - 64 )), 64.f, 64.f, 0, rand() % 9 + 1 );
	// RU: Возвращаемся к обычному рендеру.
	// EN: Return to default rendering.
	rtarget_Set( NULL );

	rtarget_Set( rtDefault );
	asprite2d_Draw( texTux, float(rand() % ( 800 - 64 )), float(rand() % ( 600 - 64 )), 64, 64, 0, rand() % 9 + 1 );
	rtarget_Set( NULL );
